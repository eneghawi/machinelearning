...
def fully_connected_layer(inputs, input_shape, output_shape, activation=tf.nn.leaky_relu):
    ...
    '''
    m=np.random.uniform(0,0.2)
    weights=tf.random_normal([input_shape,output_shape], mean=m, stddev=init_stddev(3, 128))
...
def dense_net(x,dropout):
    ...
    l9 = fully_connected_layer(d8, 2048, 1024)


    final = fully_connected_layer(l9, 1024, 10)     #for 10 classes
...
def dense_net_(x):
    ...
    l2 = fully_connected_layer(l1, 128, 256)
    
    l3 = fully_connected_layer(l2, 256, 512)
    
    l4 = fully_connected_layer(l3, 512, 1024)
    
    l5 = fully_connected_layer(l4, 1024, 512)
    
    l6 = fully_connected_layer(l5, 512, 128)
    
    l7 = fully_connected_layer(l6, 128, 1024)
    
    l8 = fully_connected_layer(l7, 1024, 2048)
    
    l9 = fully_connected_layer(l8, 2048, 1024)
...
z_ = dense_net_(X)              #no dropout here
